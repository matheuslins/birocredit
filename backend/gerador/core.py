...
Não é um código feito por mim!
Fonte: https://github.com/paoloo/geraPessoasAPI
...
def gerapessoa():
  ...
  _logradouro = '%s %s %s %s' % (logradouro[randint(0,len(logradouro)-1)],ruas[randint(0,len(ruas)-1)],nomes[0][randint(0,len(nomes[0])-1)],sobrenomes[randint(0,len(sobrenomes)-1)])
  ...
  return {'nome': _nomec, 'endereco': {'logradouro': _logradouro, 'cidade': _cidade, 'uf': _uf, 'cep': _cep, 'numero':  str(randint(1, 999))}, 'telefone': _fone, 'sexo': _sexo, 'documentos': {'cpf': _cpf, 'rg': _rg}, 'nascimento': _dt_nasc, 'mae': _mae,
          'sangue':_sangue,'cartao de credito': {'numero': _cc, 'cvv': _cvv, 'validade':_vencimentocc }, 'peso': _peso, 'altura':_altura, 'emprego':_emprego}
